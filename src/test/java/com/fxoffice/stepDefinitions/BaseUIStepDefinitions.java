...
public class BaseUIStepDefinitions {
	...
	public void before() throws Throwable {
		 ...
		 * LOGGER.info("Current Running Scenario Tag : " + currentTag); String tag[] =
		 * currentTag.split(" "); if(tag[0].contains("UAT") ||
		 * tag[0].contains("Sandbox") || tag[0].contains("PROD")) {
	...
	private void clickLogoutBtn() throws InterruptedException {
	  ...
	  private void bossLogout() throws InterruptedException {
		  Thread.sleep(7000);
			/*
			 * WebElement closeIcon =
			 * driver.findElement(By.xpath("//i[contains(@class,'glyph-icon fa fa-times')]")
			 * ); ((JavascriptExecutor)
			 * driver).executeScript("arguments[0].click();",closeIcon);
			 */
	 WebElement  accountDropDown = driver.findElement(By.
	  xpath("//i[contains(@class,'glyph-icon fa fa-angle-down')]"));
	  ((JavascriptExecutor)
	  driver).executeScript("arguments[0].click();",accountDropDown); WebElement
	  logoutIcon = driver.findElement(By.
	  xpath("//i[contains(@class,'glyph-icon fa fa-power-off')]"));
	  ((JavascriptExecutor)
	  driver).executeScript("arguments[0].click();",logoutIcon);
	  
	  
	  }
	 
	@And("I Switch To New Window Session")
	    public void set_NewWindowSession() {
		 driver.switchTo().newWindow(WindowType.TAB);
	}

	@And("I Switch Back To Parent Window Session")
    public void set_ParentWindowSession() {
		driver.switchTo().window(ParameterService.getParameter("ParentWindowSessionID"));
	}

	@And("I Switch Back To Child Window Session")
    public void set_ChildWindowSession() {
		driver.switchTo().window(ParameterService.getParameter("ChildWindowSessionID"));
	}

	@And("I Close Current Focused Window Session")
    public void set_CloseCurrentFocusedWindowSession() {
		driver.close();
		driver.switchTo().window(ParameterService.getParameter("ParentWindowSessionID"));
	}
